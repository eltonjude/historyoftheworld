...
//  version v0.1
//  http://bl.ocks.org/rengel-de/5603464#timeline.js
...
function timeline(domElement) {
    ...
    timeline.data = function(items) {
		...
		function collapseItems() {
			...
			data.items.forEach(function (item) {
				if(item.era) {
					var eraItem = eraMap[item.era];
					if(!eraItem) {
						eraItem = {};
						for (i in item) {
							eraItem[i] = item[i];
						}
						eraItem.label = item.era;
						if(item.instant) {
							eraItem.end = eraItem.start;
						}
						eraItem.instant = false;
						eraMap[item.era] = eraItem;
						compressed.push(eraItem);
					}
					if(item.start < eraItem.start)	eraItem.start = item.start;
					if(!item.instant && item.end > eraItem.end)	eraItem.end = item.end;
					if(item.instant && item.start > eraItem.end)	eraItem.end = item.start;
				} else {
					compressed.push(item);
				}
			});
			data.uncompressed = data.items;
    ...
    timeline.tooltips = function (bandName) {
        ...
        function getHtml(element, d) {
            var html;
            if (element.attr("class").indexOf("interval") >= 0) {
